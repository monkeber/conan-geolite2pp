...
class Geolite2Conan(ConanFile):
    ...
    def requirements(self):
        self.requires("maxminddb/1.3.2@monkeber/testing", private=False, override=False)

    def imports(self):
        self.copy("*", "include", "include", root_package="maxminddb")
    ...
    def build(self):
        ...
        print("---------------------------------------------")
        cxxflags += "-isystem\\ {}".format(self.deps_cpp_info["maxminddb"].includedirs[0])
        print(cxxflags)
        self.run("dir " + self.deps_cpp_info["maxminddb"].includedirs[0])

        cmake = CMake(self)
        cmake.configure(source_folder="geolite2++", args=["-DCMAKE_CXX_FLAGS={}".format(cxxflags)])
        cmake.build()

    def package(self):
        self.run("cd geolite2++/scripts && ./geolite2pp_get_database.sh")
        self.copy("*.mmdb", dst="bin", src="geolite2++/scripts")
        self.copy("*.hpp", dst="include", src="geolite2++/src-lib")
